...
test('generates topic navigation, stable pages, and rendered Mermaid diagrams', async () => {
  ...
  try {
    ...
    const feedbackRatings = JSON.parse(await readFile(path.join(outputDirectory, '_data', 'feedback_ratings.json'), 'utf8'));
    ...
    assert.match(explanation, /quiz_id: "kafka--partition-count-key-ordering"/);
    assert.match(explanation, /data-quiz-feedback/);
    assert.equal((explanation.match(/data-feedback-rating/g) ?? []).length, 12);
    assert.match(explanation, /🦨<\/span> <span>Code smells</);
    assert.match(explanation, /👍<\/span> <span>Good</);
    assert.match(explanation, /🙂<\/span> <span>Easy</);
    assert.match(explanation, /💡<\/span> <span>Brilliant</);
    assert.match(explanation, /🧩<\/span> <span>Overcomplicated</);
    assert.match(explanation, /❌<\/span> <span>Wrong answer</);
    assert.match(explanation, /⚠️<\/span> <span>Incorrect question</);
    assert.match(explanation, /😵‍💫<\/span> <span>Confusing description</);
